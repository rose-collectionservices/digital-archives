...
3. Create three subfolders:
 ...
 4. Within the Originals folder, create a subfolder for each disk image/batch.
    * In BitCurator: While in the Originals folder, right click and choose "Open in Terminal."
    * In the command line window that opens, type the following, replacing the second number in curly brackets with an estimated final number based on the number of media: mkdir ###\_{01..##} (for example: mkdir 1538\_{01..35}).
    * If you need a command to batch create folders in Windows, ask a digital archivist.
