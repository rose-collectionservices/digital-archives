...
1. Extent: at end add “[MB/GB amount] born-digital material ([# files])
...
5. Processing Note: "Born-digital materials processed, arranged, and described by [name], [date]. Born-digital materials include files taken from [list of items]." 
    * If finding aid does not have series, add this to the collection-level processing note (revised to reflect the processing steps you actually took): "Forensic disk images were created from the [material] using [program]. Individual files were extracted using FTK Imager and scanned for viruses using McAfee’s anti-virus software; none were found. Duplicate and system files have been removed, and files were scanned for personally identifiable information; none were found. Text-based files were migrated to PDF using Adobe Acrobat and image files were migrated to JPG using Microsoft Paint. File dates have been changed from the original creation date. Access copies retain original folder titles and file names." 
    * If finding aid has series, add this to the collection-level processing note: "For information as to how these materials were processed, see the processing note in the description of series [number], Born-digital material [or series the born-digital material is found in]." Put the more detailed information above in a series-level processing note.
